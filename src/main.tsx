...
function InternalApp() {
  ...
  return <main className="app-shell">
    <Header logoText="מאגר מיקומים" sub="מרכז המיקומים התפעולי" name={session.user.full_name} role={session.user.role==='owner'?'בעלים':'מנהל'} onLogout={logout} extra={session.user.role==='owner'?<button onClick={()=>setUsersOpen(true)} title="ניהול משתמשים"><UserCog size={18}/></button>:undefined}/>
...
function EmployeePortal(){
  ...
  return <main className="app-shell employee-shell">
    <Header logoText="מיקומים לעובדים" sub="קבוצת דנאל" name={session.employee.full_name} role="עובד" onLogout={logout}/>
    <section className="hero employee-hero"><div><p className="eyebrow">פורטל העובדים</p><h1>מוצאים מיקום ויוצאים לדרך</h1><p>חיפוש מהיר, ניווט ישיר והגשת מיקום חדש — מכל טלפון.</p></div><button className="primary" onClick={()=>setRequestOpen(true)}><Send size={18}/> בקשה להוספת מיקום</button></section>
    <section className="panel">
...
function LocationModal({item,onClose,onSave,requestMode=false}:{item:LocationItem|LocationDraft;onClose:()=>void;onSave:(i:LocationItem|LocationDraft)=>void;requestMode?:boolean}){
 ...
 return <div className="modal-backdrop"><form className="modal" onSubmit={e=>{e.preventDefault();onSave(form)}}><div className="modal-head"><div><p className="eyebrow">{requestMode?'בקשת עובד':'ניהול מיקום'}</p><h2>{requestMode?'בקשה להוספת מיקום':'id'in item?'עריכת מיקום':'הוספת מיקום חדש'}</h2></div><button type="button" onClick={onClose}>×</button></div><div className="form-grid"><label>מאגר<select value={form.category} onChange={e=>set('category',e.target.value)}><option value="work_site">אתרי עבודה</option><option value="reporting_point">נקודות דיווח</option></select></label><label>סוג<select value={form.place_type} onChange={e=>set('place_type',e.target.value)}><option value="station">תחנה</option><option value="segment">קטע</option></select></label><label className="full">שם המיקום<input value={form.name} onChange={e=>set('name',e.target.value)} placeholder="לדוגמה: קטע אשדוד - ניצנים" required/></label><label>ק״מ<input value={form.km} onChange={e=>set('km',e.target.value)} placeholder="145+000"/></label><label>קואורדינטות<input value={form.coordinates} onChange={e=>set('coordinates',e.target.value)} placeholder="31.8000, 34.6500"/></label><label className="full">קישור Waze<input value={form.waze_url} onChange={e=>set('waze_url',e.target.value)} placeholder="https://waze.com/ul?..."/></label><label className="full">קישור Google Maps<input value={form.maps_url} onChange={e=>set('maps_url',e.target.value)} placeholder="https://maps.google.com/..."/></label><label className="full">הערות<textarea value={form.notes} onChange={e=>set('notes',e.target.value)} rows={3}/></label></div><div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>ביטול</button><button className="primary">{requestMode?'שליחת בקשה':'שמירת מיקום'}</button></div></form></div>
}
...
createRoot(document.getElementById('root')!).render(<React.StrictMode>{isEmployeePortal?<EmployeePortal/>:<InternalApp/>}</React.StrictMode>);
