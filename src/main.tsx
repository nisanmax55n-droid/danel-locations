...
function InternalApp() {
  ...
  return <main className="app-shell">
    <Header logoText="מאגר מיקומים" sub="מרכז המיקומים התפעולי" name={session.user.full_name} role={session.user.role==='owner'?'בעלים':'מנהל'} onLogout={logout} extra={<>{portalButton}{session.user.role==='owner'&&<button onClick={()=>setUsersOpen(true)} title="ניהול משתמשים"><UserCog size={18}/></button>}</>}/>
